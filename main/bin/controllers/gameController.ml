  ...
  let rec game_loop var_game last_time name msgs = 
    ...
    if window_should_close () then begin
      ...
      begin
        ...
        let (player, select, loots, enemy, traps_and_grounds, last_time, msg, my_map) = update_player player enemy my_map select loots traps_and_grounds last_time in
    ...
    if window_should_close () then
      ...
      if screen_state = Game then
        begin          
          ...
          let list_of_last_time = List.init (100 + ((List.length(enemy))*4)) (fun _ -> 0.0) in (* Use for animations *)
          let msgs = [] in
