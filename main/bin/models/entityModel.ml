    ...
    let rec aux player enemy acc msg =
      ...
      | e :: rest ->
        ...
        if int_of_float e_pos_x = int_of_float target_x && int_of_float e_pos_y = int_of_float target_y then begin
          let degat = calcul_degats player e (List.nth player.competence (Random.int (List.length player.competence))) in
            let msg = Printf.sprintf "%s inflige %d dégâts à %s !\n%!" player.nom degat e.nom in
